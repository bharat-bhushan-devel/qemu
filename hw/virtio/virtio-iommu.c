...
static uint64_t virtio_iommu_get_ep_page_size_mask(VirtIOIOMMUEndpoint *ep)
{
    VirtIOIOMMU *s = ep->viommu;
    IOMMUDevice *sdev;
...
{
    struct virtio_iommu_probe_pgsize_mask *prop_buf;
    size_t prop_size = sizeof(*prop_buf);
    uint64_t page_size_mask;

    page_size_mask = virtio_iommu_get_ep_page_size_mask(bufstate->endpoint);

    /*
     * Not filling page_size_mask property when page-size-mask for endpoint
     * not set. Return false to continue traversal.
     */
    if (!page_size_mask) {
           return false;
    }
    ...
    prop_buf = (struct virtio_iommu_probe_pgsize_mask *)
                     (bufstate->start + bufstate->filled);

    prop_buf->head.type = VIRTIO_IOMMU_PROBE_T_PAGE_SIZE_MASK;
    prop_buf->head.length = prop_size;
    prop_buf->pgsize_bitmap = page_size_mask;
    bufstate->filled += sizeof(*prop_buf);
    trace_virtio_iommu_fill_pgsize_mask_property(bufstate->endpoint->id,
                                                 prop_buf->pgsize_bitmap,
                                                 bufstate->filled);
    return false;
}
